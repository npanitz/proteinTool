...
class FrontPage extends React.Component {
  ...
  onFormSubmit(event) {
    ...
    fetch("http://localhost:2000/protein/create", {
      headers: {
    ...
    return (
      <div className="main-container">
        <ProteinCreator
          ...
          updateProteinName={(ev) => {
            this.updateProteinName(ev.target.value);
          }}
  ...
  return (
    <div className="ui celled list">
      {proteins.map((protein) => (
        <div className="item">
          <Link
            className="content protein-item"
            to={`/protein/${protein.name}`}
            key={protein.name}
          >
            <div>{util.firstLetterUpperCase(protein.name)}</div>
          </Link>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="create-container">
      <form onSubmit={props.onFormSubmit} className="ui form">
        ...
        />
        <input
          className="ui button create-button"
          type="Submit"
